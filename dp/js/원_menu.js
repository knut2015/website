...
function viewOrderListBlock(isState){
	$(".orderToggle").bind("click", function(){
		if (!isState){
			$("#orderStream").stop().animate({bottom: -$("#orderStream").height() + 127+'px'}, 500, 'easeOutExpo');
			$(this).css({'background': 'url("./img/orderToggle_bottom.png") 0 0 no-repeat'});
			isState = true;
		}else{
			$("#orderStream").stop().animate({bottom: '340px'}, 400, 'easeOutExpo');
...
function scrollList(distance, duration)
{
	...
	$('body').append("<div style='position: fixed; font-size:2em; color: #FF0000; top: 0; left: 0; background: #FFF; z-index: 9999;'>"+Math.round(value)+"</div>");
}
