...
function creatGnb(){
	...
	var drinkMenu = "./res/drink.xml";
	var eventMenu = "./res/event.xml";
	dataLoad(foodMenu);
...
function responseData(data){
	...
	$(data).find('section').each(function(i){
		...
		console.log($(".p" + i + " div.mType").text());
		
		if ($(".p" + i + " div.mType").text() == ""){
			$("li.cate").css({ 'margin-top': '-20px', 'display': 'block', 'position': 'relative', 'z-index': '0'});
			$("div.mType").css({'display': 'inline-block', 'margin': '0','color': 'red' });
		}else{
			$("li.cate").css({ 'display': 'block', 'position': 'relative', 'z-index': '0'});
			$("div.mType").css({'display': 'inline-block', 'font-size': '1.8em', 'color': 'gray', 'font-weight': 'bold', 'letter-spacing': '0.25em', 'margin': '20px 0'  });
		}
	});
...
function viewAddMenu(){
	...
	$(getData()).find('section').each(function(i){
		...
		$(this).find('menuType').find('item').each(function(){
			...
			/** menuCodeList 
			$(this).attr('code').each(function(){
				var codeList = $(this).text();
				menuCodeList.push(codeList);
			});
			**/
			/** menuList **/
...
function orderNode(){
	...
	for (var i = 0; i < orderListData.length; i++)
	{
		var dataContent = String(
			'<Order>' +
			'	<Code>' + orderListData[i].selectCode + '</Code>' +
